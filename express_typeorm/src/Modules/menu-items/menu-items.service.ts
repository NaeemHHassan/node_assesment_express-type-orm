...
export class MenuItemsService {
    ...
    */
    ...
    getMapedMenuItems(items: any[], parentId: null): any {
        const mapedManuItemsList = [];

        const childMenuitems = !parentId ? items.filter((itm) => !itm.parentId) : items.filter((itm) => itm.parentId == parentId);

        for (let item of childMenuitems) {
            mapedManuItemsList.push({ ...item, children: this.getMapedMenuItems(items, item.id) });
        }

        return mapedManuItemsList;
    }
    ...
    async getMenuItems() {
        ...
        return this.getMapedMenuItems(menuItems, null);
